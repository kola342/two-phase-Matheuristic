...
vector<double> Solution::weightLS;
double Solution::relaxation;
...
void Solution::greedyConstructNoiseCluster() {
	...
	for (auto& cluster : this->info.at_clusters()) {
		...
		for (auto customer : cluster) {
			if (this->info.at_custDelay(customer) == -1) this->custInsert.insert(customer);
	...
	for (auto it = next(solu_start); it != solu_end; ++it) {
		...
		if (pre->numDrone == 0) {//插入在无人机跨度的卡车路线
			...
			for (; end != it; ++end) {//降落点it前的
				...
				double span_drone = last->t + this->info.at_TD(last->id, end->id);//无人机到达降落点的时间
		...
		else if (!pre->droneRoute.empty() && pre->droneRoute.front().q + this->info.at_demand(obj) <= QDd) {//插入无人机路线的情况，pre作起点
		...
		if (num < obj) {//违反约束的情况
			...
			for (auto it1 = next(it); it1 != this->solu.end(); it1++) {
				if (this->info.in_CD(it1->id) && it1->droneRoute.empty() 
					&& it1->drone_end == nullptr && this->info.at_custDelay(it1->id) == 0) {//需要至少满足非起降点的要求
					auto pre = prev(it1);
					auto nex = next(it1);
	...
	for (auto it = obj; it != this->solu.end(); it++) {
		...
		if (!it->droneRoute.empty()) {//该点是起始点，计算延迟时间			
			for (auto it1 = next(it->droneRoute.begin()); it1 != prev(it->droneRoute.end()); it1++) {
				td = max(td, this->info.at_custDelay(it1->id));//延迟时间计算
			}
		}
		...
		if (type == 'b') {
			...
			for (auto it1 = next(it->droneRoute.begin()); it1 != it->droneRoute.end(); it1++) {
				if (it1->id != best_end->id)
					...
					it1->t = prev(it1)->t + this->info.at_TD(prev(it1)->id, it1->id);
			}
		}
		else if (type == 'w') {
			double delta = it->droneRoute.front().t;//起飞的时间变化量
...
void Solution::calMakespan_resupply(map<int, DroneResupply>& res, ResupplyInfo& result) {
	...
	for (auto it = this->solu.begin(); it != this->solu.end(); ++it) {
		...
		if (resupply != res.end()) {//增加补货方案，针对于补货点
			it->resupply = move(resupply->second);
...
void Solution::solveResupply() {
	...
	resupply.solve(res);
	this->calMakespan_resupply(res, resupply);
...
void Solution::handleRemoveCustomer(int id) {
	if (this->info.at_custDelay(id) != 0 || this->tw_resupply.find(id) != this->tw_resupply.end()) {
...
void Solution::randomRemove() {
	int num = this->info.get_num();//客户总数
	...
	while (this->custInsert.size() + this->custInsert_delay.size() < this->numRemove) {
		...
		this->remove(loc);
		int yy = cal();
		if (this->custInsert.size() + this->custInsert_delay.size() + yy != 100) {
			throw runtime_error("111");
		}
		yy = call();
		if (yy != 100) {
...
int Solution::cal() {
	...
	for (auto it = this->solu.begin(); it != solu.end(); ++it) {
		...
		yy += it->droneRoute.size() - 2;
	}
	return yy + this->solu.size() - 2;
}
int Solution::call() {
	set<int> temp;
	...
	while (this->custInsert.size() + this->custInsert_delay.size() < this->numRemove) {
		...
		for (int i = 1; i <= num; i++) {
			if (this->custInsert.find(i) != this->custInsert.end()
				...
				continue;
			shaw.emplace_back(i, this->info.at_dis(obj, i));
		}
